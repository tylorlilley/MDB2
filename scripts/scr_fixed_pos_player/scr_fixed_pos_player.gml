...
function update_player_state() {
	...
	if (transition_timer > 0) {
		...
		switch (state) {
			...
			case PLAYER_STATES.PUSH_WALK: {
				virtual_x += (is_left) ? -2 : 2; break;
				if (instance_exists(pushed_obj)) { pushed_obj.virtual_x += (is_left) ? -2 : 2; } break;
			}
		...
		switch (state) {
			...
			case PLAYER_STATES.POWERCROUCH: {
				if (!_on_ground) {
					...
					*/
					if ((key_up && can_ladder_up()) || (key_down && can_ladder_down())) { state = PLAYER_STATES.LADDER; }
					else if (key_left || key_right) {
						is_left = key_left;
						
						if (prev_state == PLAYER_STATES.WALK && is_left != _prev_is_left) {
							...
							if (_can_walk) {
								...
								var _can_climb = false; _can_push = false; // key_up
								...
								if (_can_climb) {
									...
									if (key_up && _pushing_on_wall && !_under_ceiling) {
										transition_timer = 12;
										y -= 16;
									...
									ds_list_destroy(_wall_objects);
									// TODO
									if (_pushing_on_wall) { state = PLAYER_STATES.PUSH_WALK; } // TODO
									else { transition_timer = 4; x += (is_left) ? -8 : 8; }
								}
								else {
	...
	if (cape_timer == 0) {
		if (cape_state == CAPE_STATES.FLUTTER || cape_state == CAPE_STATES.TURN) {
			if (state == PLAYER_STATES.WALK || state == PLAYER_STATES.PUSH_WALK) {
		...
		else if (cape_state == CAPE_STATES.STOP_FLUTTER || cape_state == CAPE_STATES.STAND || cape_state == CAPE_STATES.CROUCH) {
			...
			else if (state == PLAYER_STATES.STAND || PLAYER_STATES.PUSH) {
				cape_state = CAPE_STATES.STAND;
				cape_sprite_index = spr_cape_stand;
				cape_image_index = 0;
				cape_timer = 0;
		...
		else if (cape_state == CAPE_STATES.FLY) {
		...
		else if (cape_state == CAPE_STATES.FALL_START) {
		...
		else if (cape_state == CAPE_STATES.FALL) {
	...
	if (state != prev_state) {
		...
		else if (state == PLAYER_STATES.FALL && prev_state == PLAYER_STATES.POWERFALL) {
			cape_state = CAPE_STATES.FALL_START;
			cape_sprite_index = spr_cape_fall_start;
			cape_image_index = 0;
			cape_timer = 4;
...
function update_player_graphics() {
	...
	if (prev_state != state) {
		...
		switch (state) {
			...
			case PLAYER_STATES.WALK: {
				...
				if (prev_state != PLAYER_STATES.WALK && prev_state != PLAYER_STATES.PUSH && prev_state != PLAYER_STATES.PUSH_WALK) { image_index = 0; }
				break;
			}
			case PLAYER_STATES.PUSH_WALK:
			case PLAYER_STATES.PUSH: {
	...
	switch (state) {
		...
		case PLAYER_STATES.PUSH:
		case PLAYER_STATES.PUSH_WALK: { if (animation_timer % 8 == 0) { image_index++; } break; }
		case PLAYER_STATES.LADDER_UP:
		case PLAYER_STATES.LADDER_DOWN:
		case PLAYER_STATES.LAND:
...
/*
		switch (state) {
			...
			// Handle special cases
	if (state == PLAYER_STATES.STAND) { sprite_index = (key_up) ? spr_player_look_up : spr_player_walk; }
	if (state == PLAYER_STATES.FALL && fall_timer == 10) {
		sprite_index = spr_player_tumble;
		image_index = 0;
	}
	if (state == PLAYER_STATES.HOP && image_index == 3) {
		image_speed = 0;
	}
		*/